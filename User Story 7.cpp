// User Story 7.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
// logs the session duration and printing cost to a separate session_logs.txt file per user
void logSession(const string& userId, int minutes, int seconds, double printCost) {
    ofstream logFile("session_logs.txt", ios::app);  // Append mode to change log
    if (logFile.is_open()) {
        time_t now = time(0);
        tm localTime;
        localtime_s(&localTime, &now);
        char buffer[80];
        strftime(buffer, sizeof(buffer), "%d/%m/%Y %H:%M:%S", &localTime); // format: day/month/year ?? not sure if this is working correctly..ideally want this to show the date properly

        logFile << "UserID: " << userId
            << " | Duration: " << minutes << " min " << seconds << " sec"
            << " | Print Charges: $" << fixed << setprecision(2) << printCost
            << " | Date: " << buffer << "\n";
...
// logs the print job details to print_logs.txt
...
// lets user print documents during session, records page count and updates session print cost
void printDocuments(const string& userId, double& totalPrintCost) {
    ...
    totalPrintCost += cost;  // add to session's print cost

    logPrintJob(userId, pages, cost);  // log individual print job

    cout << "Printed " << pages << " pages. Total cost: $" << fixed << setprecision(2) << cost << "\n";
}

// provides user services menu after login with options to check session time, view session logs, print, or logout
void accessServices(const string& userId, const steady_clock::time_point& sessionStartTime) {
    double totalPrintCost = 0.0;  // stores print charges for this session
    ...
    while (true) {
        ...
        if (choice == "1") {
            ...
            logSession(userId, minutes, seconds, totalPrintCost);

            cout << "Logging out. Session lasted: " << minutes << " min, " << seconds << " sec.\n";
            cout << "Print charges this session: $" << fixed << setprecision(2) << totalPrintCost << "\n";
            break;
        }
        else {
            cout << "Invalid option. Please choose 1, 2, 3, or 4.\n";
        }
    }
}
