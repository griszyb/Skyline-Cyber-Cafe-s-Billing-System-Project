// Finished Product.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
vector<string> adminEmails = {
    "admin@skylinecafe.com", //admin emails
...
// validates password with best practice
...
// admin function: view system-wide usage summary
void adminSystemUsageSummary() {
    int totalSessionSeconds = 0;
    int totalPrintPages = 0;
    int totalScanItems = 0;
    double totalPrintValue = 0.0;
    double totalScanValue = 0.0;
    ...
    while (getline(printFile, line)) {
        ...
        if (costPos != string::npos) {
            double cost = 0.0;
            sscanf_s(line.c_str() + costPos, "Cost: $%lf", &cost);
            totalPrintValue += cost;
        }
    }
    printFile.close();
    ...
    cout << "Total printed pages: " << totalPrintPages << " (Value: $" << fixed << setprecision(2) << totalPrintValue << ")\n";
    cout << "Total scanned items: " << totalScanItems << " (Value: $" << fixed << setprecision(2) << totalScanValue << ")\n";
    cout << "----------------------------------\n";
}
...
void registerAdmin() {
    ...
    if (!isAdminEmail(email)) {
        cout << "Not an authorized admin email.\n";
        return;
    }
...
void adminMenu() {
    while (true) {
        ...
        cout << "4. View system-wide usage summary\n";
        cout << "5. Logout\n";
        cout << "Choose an option: ";
        int option;
        cin >> option;
        ...
        if (option == 4) {
            adminSystemUsageSummary();
        }
        else if (option == 5) {
            // logout or exit admin menu
        }
        else {
            cout << "Invalid option.\n";
        }
